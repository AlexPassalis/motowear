...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await Promise.all(
      Object.entries(validatedBody.values).map(array =>
        limit(async () => {
          const [{ maxIndex }] = await postgres
            .select({ maxIndex: max(review.index) })
            .from(review)
            .where(eq(review.product_type, array[0]))
  ...
  return NextResponse.json(
    { coupon_code: 'REVIEW-15', percentage: 0.15, fixed: null } as typeCoupon,
    { status: 200 }
  )
}

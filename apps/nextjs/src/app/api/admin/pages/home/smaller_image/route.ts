...
export async function DELETE(req: NextRequest) {
  ...
  try {
    validatedBody = z
      .object({
        smaller_image: z.string(),
      })
      .parse(await req.json())
  } catch (e) {
    const message = formatMessage(
      '@/app/api/admin/pages/home/smaller_image/route.ts DELETE',
      errorInvalidBody,
      e,
    )
  ...
  try {
    await postgres.execute(
      sql`
        UPDATE ${home_page}
        SET smaller_images = COALESCE((
          SELECT jsonb_agg(elem)
          FROM jsonb_array_elements(${home_page}.smaller_images) AS elem
          WHERE elem->>'image' <> ${validatedBody.smaller_image}
        ), '[]'::jsonb)
        WHERE ${home_page}.smaller_images @> ${JSON.stringify([
        { image: validatedBody.smaller_image },
      ])}
      `,
    )
  } catch (e) {

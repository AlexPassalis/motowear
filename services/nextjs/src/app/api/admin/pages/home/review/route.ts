...
export async function DELETE(req: NextRequest) {
  ...
  const requestBodySchema = z.object({ review_id: z.string() })
  ...
  try {
    ...
    SET reviews = COALESCE(
      (
        ...
        FROM jsonb_array_elements(${home_page}.reviews) AS elem
        WHERE elem->>'id' <> ${validatedBody.review_id}
      ),
      '[]'::jsonb
    )
    WHERE ${home_page}.reviews @> ${JSON.stringify([
        { id: validatedBody.review_id },
      ])}
  `,
    )
  } catch (err) {
    const location = 'DELETE POSTGRES update home_page reviews'
    handleError(location, err)
